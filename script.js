...
function analyzeWinner (firstArea, secondArea, thirdArea) {
    if(firstArea.firstChild.className == secondArea.firstChild.className && firstArea.firstChild.className == thirdArea.firstChild.className){
        ...
        const classElement = firstArea.firstChild.className;
        const namePlayer1 = document.querySelector('#input-player01').value;
        const namePlayer2 = document.querySelector('#input-player02').value;
        if(classElement == 'x-icon') {
            alert(`${namePlayer1} ganhou a partida!`);
            resartGame();
            scorePlayer1.innerText = Number(scorePlayer1.innerText) + 1;
        } else {
            alert(`${namePlayer2} ganhou a partida!`);
            resartGame();
    ...
    area.addEventListener('click', (eve) => {
        ...
        if(numberOfMoves >= 5){
            if(area1.childNodes.length > 0 && area5.childNodes.length > 0 && area9.childNodes.length > 0 ){
                //Verificando transversal esquerda
                analyzeWinner(area1, area5, area9);
            }
            if (area3.childNodes.length > 0 && area5.childNodes.length > 0 && area7.childNodes.length > 0 ){
                //Verificando transversal direita
                analyzeWinner(area3, area5, area7);
            } 
            if (area4.childNodes.length > 0 && area5.childNodes.length > 0 && area6.childNodes.length > 0 ){
                analyzeWinner(area4, area5, area6);
            } 
            if (area1.childNodes.length > 0 && area2.childNodes.length > 0 && area3.childNodes.length > 0 ){
                analyzeWinner(area1, area2, area3);
            } 
            if (area7.childNodes.length > 0 && area8.childNodes.length > 0 && area9.childNodes.length > 0 ){
                analyzeWinner(area7, area8, area9);
            } 
            if( area1.childNodes.length > 0 && area4.childNodes.length > 0 && area7.childNodes.length > 0 ){
                analyzeWinner(area1, area4, area7);
            } 
            if(area2.childNodes.length > 0 && area5.childNodes.length > 0 && area8.childNodes.length > 0 ){
                analyzeWinner(area2, area5, area8);
            } 
            if (area3.childNodes.length > 0 && area6.childNodes.length > 0 && area9.childNodes.length > 0 ){
